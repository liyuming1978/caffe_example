...
XCBShow::XCBShow(){
  ...
  xcb_xv_query_extension_cookie_t ck = xcb_xv_query_extension (conn);	
	...
	if (r == NULL){
		LOG(FATAL) << "XVideo extension not available";
		xcb_disconnect(conn);
		return;
	}
	else{
		if (r->major != 2 || r->minor < 2)
			LOG(FATAL) << "XVideo extension version is not 2.2";
		else 
			binit = true;
		free (r);	
	}
			 ...
			 xcb_xv_adaptor_info_next (&it))
	{
		...
		for (unsigned i = 0; i < a->num_ports; i++)
		{
				 ...
				 LOG(INFO) << "cannot grab port " << fport;
		}
...
void XCBShow::create_win(int id){
	...
	if (err)
	{
			LOG(FATAL) << "creating window: X11 error " << err->error_code;
			free (err);
			xcb_xv_ungrab_port (conn, fport, XCB_CURRENT_TIME);
			binit = false;
			return;
