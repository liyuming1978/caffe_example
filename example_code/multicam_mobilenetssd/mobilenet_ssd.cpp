...
void *thr_camera_each(void *arg)
{
	...
	if (!cap.isOpened()) {
		LOG(INFO) << "can not open camera" << camid << " just fake it.\n";
		cap.release(); 
		iscam = false;
	}
	else{
...
void *thr_camera(void *arg) {
	...
	for(i=0;i<INPUTNUM;i++){
		cap[i].open(i);
		if (!cap[i].isOpened()) {
			LOG(INFO) << "can not open camera" << i << " just fake it.\n";
			cap[i].release(); 
		}	
		else {
...
void *thr_fps(void *arg)
{
	...
	while(grunning){
		safesleep(1000*2);//2s
		detect_timer.Stop();
		double timeUsed = detect_timer.MilliSeconds();
